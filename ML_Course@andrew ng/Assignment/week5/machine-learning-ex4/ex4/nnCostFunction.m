...
J=J/m; %non regularized cost
reg = (lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)));
J=J+reg
% backpropagation start here
sig3 = h.-Y;
sig2 = (sig3*Theta2);
sig2 = sig2.*sigmoidGradient([ones(size(z2,1),1) z2]);
sig2 = sig2(:,2:end);
...
%((lambda/m)*[zeros(size(Theta1, 1), 1) Theta1(:, 2:end)]); this is regularization
%with out Above factor gradient calculate normal
